...
import socket
import platform
from quadruped_peripheral_interfacing.msg import ElectricalMeasurements
...
from quadruped_control.Controller import Controller
from quadruped_input_interfacing.InputInterface import InputInterface
from quadruped_control.State import State, BehaviorStateped_servo_interfacing.HardwareInterface import HardwareInterface
    from quadruped_peripheral_interfacing.IMU import IMU
    from quadruped_control.Config import Leg_linkage
...
class quadrupedDriver:
    def __init__(self,is_sim, is_physical, use_imu):
        ...
        if self.is_sim:
            self.sim_command_topics = ["/quadruped_controller/FR_theta1/command",
                    "/quadruped_controller/FR_theta2/command",
                    "/quadruped_controller/FR_theta3/command",
                    "/quadruped_controller/FL_theta1/command",
                    "/quadruped_controller/FL_theta2/command",
                    "/quadruped_controller/FL_theta3/command",
                    "/quadruped_controller/RR_theta1/command",
                    "/quadruped_controller/RR_theta2/command",
                    "/quadruped_controller/RR_theta3/command",
                    "/quadruped_controller/RL_theta1/command",
                    "/quadruped_controller/RL_theta2/command",
                    "/quadruped_controller/RL_theta3/command"]
...
def main():
    """Main program
    """
    rospy.init_node("quadruped_driver") 
    signal.signal(signal.SIGINT, signal_handler)
    quadruped = quadrupedDriver(is_sim, is_physical, use_imu)
    quadruped.run()
