...
if len(sys.argv) > 1 and sys.argv[1] in servo_dict:
    ...
    if servo_name != "relax":
        print('quadruped: Motor ' + servo_name + ' moved to ' + servo_move + '.\n')
    else:
        print('quadruped: Motors Relaxed.\n')

if servo_name == "fr" or servo_name == "all":
    quadruped.moveAbsAngle(quadruped.front_right_hip  ,offsets[0,0]+pos[0])
    quadruped.moveAbsAngle(quadruped.front_right_upper,offsets[1,0]+pos[1])
    quadruped.moveAbsAngle(quadruped.front_right_lower,offsets[2,0]+pos[2])

if servo_name == "fl" or servo_name == "all":
    quadruped.moveAbsAngle(quadruped.front_left_hip   ,offsets[0,1]+pos[0])
    quadruped.moveAbsAngle(quadruped.front_left_upper ,offsets[1,1]+pos[1])
    quadruped.moveAbsAngle(quadruped.front_left_lower ,offsets[2,1]+pos[2])

if servo_name == "br" or servo_name == "all":
    quadruped.moveAbsAngle(quadruped.back_right_hip   ,offsets[0,2]+pos[0])
    quadruped.moveAbsAngle(quadruped.back_right_upper ,offsets[1,2]+pos[1])
    quadruped.moveAbsAngle(quadruped.back_right_lower ,offsets[2,2]+pos[2])

if servo_name == "bl" or servo_name == "all":
    quadruped.moveAbsAngle(quadruped.back_left_hip    ,offsets[0,3]+pos[0])
    quadruped.moveAbsAngle(quadruped.back_left_upper  ,offsets[1,3]+pos[1])
    quadruped.moveAbsAngle(quadruped.back_left_lower  ,offsets[2,3]+pos[2])

if servo_name == "relax":
    quadruped.relax_all_motors()
